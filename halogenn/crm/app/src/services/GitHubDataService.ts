...
const LS_CONFIG = 'hal_config_v1';
const DEFAULTS = { owner: 'Plinio19', repo: 'gestor-ads', branch: 'main' };

const CACHE_MAP: Record<string, string> = {
  'halogenn/crm/data/clientes.json':   'hal_clientes',
  'halogenn/crm/data/negocios.json':   'hal_negocios',
  'halogenn/crm/data/atividades.json': 'hal_atividades',
};
...
class GitHubDataService {
  ...
  rawUrl(path: string): string {
    const cfg = this.getConfig() ?? { ...DEFAULTS, token: '' };
    return `https://raw.githubusercontent.com/${cfg.owner}/${cfg.repo}/${cfg.branch ?? DEFAULTS.branch}/${path}`;
  }
  ...
  async getCollection<T>(path: string): Promise<DataResult<T>> {
    ...
    try {
      ...
      if (cacheKey) {
        ...
        if (cached) {
          const parsed = JSON.parse(cached) as DataResult<T>;
          return parsed;
        }
      }
      throw err;
