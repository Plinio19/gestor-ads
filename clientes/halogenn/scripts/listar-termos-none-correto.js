/**
 * Garimpo de termos NONE — LAST_30_DAYS, seguindo mine-search-terms skill exatamente
 * Filtro: impressions > 10 | Ordem: custo DESC | Inclui keyword que disparou
 */
...
async function listar() {
  ...
  console.log('\n=== TERMOS NONE — LAST 30 DAYS (skill correta) ===\n');
  ...
  if (!rows.length) {
    console.log('Nenhum termo com status NONE e > 10 impressões nos últimos 30 dias.\n');
    return;
  }

  console.log(`Total de termos NONE (>10 impr): ${rows.length}\n`);
  console.log('-'.repeat(110));
  console.log(
    `${'#'.padEnd(4)} ${'TERMO DE BUSCA'.padEnd(48)} ${'AD GROUP'.padEnd(28)} ${'IMPR'.padStart(5)} ${'CLI'.padStart(4)} ${'CTR'.padStart(7)} ${'CUSTO'.padStart(9)} ${'CONV'.padStart(5)}`
  );
  console.log('-'.repeat(110));
  ...
  rows.forEach((r, i) => {
    const termo     = r.search_term_view.search_term.substring(0, 41);
    ...
    console.log(
      `${String(i + 1).padEnd(4)} ${termo.padEnd(48)} ${ag.padEnd(28)} ${String(impr).padStart(5)} ${String(clicks).padStart(4)} ${ctr.padStart(7)} ${custo.padStart(9)} ${String(conv).padStart(5)}`
    );
  });
